...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
W=logspace(-2,4,100);
gain=1/10000; f1=1; f2=10;
    w1=2*pi*f1; w2=2*pi*f2; gain=gain*(w2/w1);
    num=[1 w1]; den=[1 w2];
    sys = tf(gain*num,den);
...
% -------------------------------- wsV ---------------------------------% 
gain=5; f1=0.1;f2=0.5;  w1=2*pi*f1; w2=2*pi*f2;
    num=[1 w2]; den=[1 w1];
    sys = tf(gain*num,den);
...
% -------------------------------- wsPs ---------------------------------% 
gain=3;    f1=1;f2=10;     w1=2*pi*f1; w2=2*pi*f2;
    num=[1 w2]; den=[1 w1];num=1;den=1;gain=1;
    sys = tf(gain*num,den);
...
gain=3; f1=0.1;f2=0.5;  w1=2*pi*f1; w2=2*pi*f2;
    num=[1 w2]; den=[1 w1];
    sys = tf(gain*num,den);
    magU = bode(sys,W); magU = squeeze(magU); magU = 20*log10(magU); 
    wsU = nd2sys(num,den,gain);
...
% Ws = daug(wsX,wsY,wsZ,wsV,wsPh,wsTh,wsPs);
%{%
...
figure
set(gcf, 'Name', 'Performance Weighting Function');
semilogx(W/(2*pi),magX,W/(2*pi),magY,W/(2*pi),magU, W/(2*pi),magV);
xlabel('Frequency [rad/s]'); ylabel('Gain [dB]');
legend('{\itW_x}','{\itW_y}','{\itW_u}','{\itW_v}'); grid on; xlim([10^(-2) 10^(1)]);ylim([-20 60]);
